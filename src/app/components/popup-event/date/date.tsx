...
export default function DateField({
  ...
  useEffect(() => {
    function handleClickOutside(event: MouseEvent) {
      if (
        beginningHoursRef.current &&
        !beginningHoursRef.current.contains(event.target as Node)
      ) {
        setIsBeginningHoursVisible(false);
      }
      if (
        endHoursRef.current &&
        !endHoursRef.current.contains(event.target as Node)
      ) {
  ...
  return (
    <div className="flex items-center pt-4 text-dark-pink">
      <button className="hover:bg-very-light-pink p-1 text-left flex-wrap whitespace-nowrap rounded-lg">
        {capitalizeFirstLetter(
          popupDate.toLocaleDateString("fr-FR", { weekday: "long" })
        )}
        , {popupDate.getDate()}{" "}
        {capitalizeFirstLetter(
          popupDate.toLocaleDateString("fr-FR", { month: "long" })
        )}{" "}
      ...
      <div>
        ...
        <div
          ...
          className={`absolute border border-gray-200 rounded-lg h-52 overflow-scroll flex-col bg-white mt-1 ${
            isBeginningHoursVisible ? "flex" : "hidden"
          }`}
          id="beginningHours"
          style={{ zIndex: 10 }}
        >
      ...
      <div className="w-full">
        ...
        <div
          ref={endHoursRef}
          className={`absolute border border-gray-200 rounded-lg max-h-52 overflow-scroll flex-col bg-white mt-1 ${
            isEndHoursVisible ? "flex" : "hidden"
          }`}
          id="endHours"
          style={{ zIndex: 10 }}
        >
          {Array.from(
            { length: Math.min(14, 20 - popupDate.getHours() - 1) },
            (_, i) => i + popupDate.getHours() + 1
          ).map((hour, index) => (
            <button
              key={hour + index + "end"}
              className="hover:bg-gray-100 p-2 text-left w-32 hover:bg-very-light-pink"
