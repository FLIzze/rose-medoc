...
export default function SideMonthlyCal() {
  ...
  const firstDayOfMonth =
    (new Date(currentYearCal, currentMonthCal, 1).getDay() + 6) % 7;
  ...
  const handlePreviousMonth = () => {
    setSidebarDate(
      new Date(sidebarDate.getFullYear(), sidebarDate.getMonth() - 1, 1)
    );
  };
  ...
  return (
    <div className="pb-6 h-64">
      <div className="flex justify-between items-center pl-2 text-sm font-bold">
        <h2 className="w-full text-dark-pink">
          {capitalizedMonthName} {currentYearCal}
        </h2>
      ...
      <div className="grid grid-cols-7 text-sm">
        {daysOfWeek.map((day, index) => (
          <div key={day + index} className="p-1 text-center font-bold">
            <p className="text-xs text-dark-pink">{day}</p>
          </div>
        ))}
        ...
        {daysArray.map((date) => {
          ...
          return (
            ...
            >
              <p
                className={`p-2 text-xs rounded-full transition-all ${
                  hasEvents
                    ? "text-white bg-medium-pink hover:bg-dark-pink"
                    : "text-medium-pink hover:bg-very-light-pink"
                }`}
              >
                {date.getDate()}
              </p>
